...
namespace EuCorro.MVC.Site.Areas.Vendas.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    var usuario = _userManager.FindByEmail(model.Email);
                    new AutenticacaoVendas()
                        .Autenticar(new Autenticacao.UsuarioEmail
                        {
                            Codigo = usuario.Id.ToString(),
        ...
        public async Task<ActionResult> Register(CadastroViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                //var user = new ApplicationUser { UserName = model.Email, Email = model.Email, PerfilUsuarioId = 3 };
                var cliente = new ApplicationUser();
                cliente.PerfilUsuarioId = 3;
                cliente.Email = model.Email;
                cliente.PasswordHash = model.Senha;
                cliente.UserName = model.Nome.ToUpper();
                cliente.Numero = model.Numero;
                cliente.DataNascimento = model.DataNascimento;
                cliente.Idade = DateTime.Now.Year - model.DataNascimento.Year;
                cliente.CPF = model.CPF;
                cliente.Camiseta = model.Camiseta;
                cliente.Brasileiro = model.Brasileiro;
                cliente.Sexo = model.Sexo;
                cliente.Contato = model.Contato;
                cliente.FoneContato = model.FoneContato;
                ...
                var result = await _userManager.CreateAsync(cliente, model.Senha);
                if (result.Succeeded)
                {
                    await _signInManager.SignInAsync(cliente, isPersistent: false, rememberBrowser: false);

                    var code = await _userManager.GenerateEmailConfirmationTokenAsync(cliente.Id);
                    var callbackUrl = Url.Action("ConfirmEmail", "Account", new { userId = cliente.Id, code = code }, protocol: Request.Url.Scheme);
                    await _userManager.SendEmailAsync(cliente.Id, "Confirme sua Conta", "Por favor confirme sua conta clicando neste link: <a href='" + callbackUrl + "'></a>");
        ...
        {
            new AutenticacaoVendas().Deslogar();
